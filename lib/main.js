...
function Protocol(opts) {
  ...
  this.skipWriteTick = (opts && typeof opts.skipWriteTick === 'boolean'
                        ? opts.skipWriteTick
                        : false);
  this.type = 0;
  this.len = 0;
...
Protocol.prototype._write = function(chunk, encoding, cb) {
  ...
  // if a lot of writes are done in a short period of time, node can blow up
  // because currently Writable's onwrite() (cb) uses nextTick() when calling
  // afterWrite(). this can cause recursion problems and even exceed the max
  // call stack size, so we provide a way to prevent this particular instance of
  // Writable's nextTick() usage here ...
  if (this.skipWriteTick)
